...
const NFTCard = ({ nft }) => {
  ...
  useEffect(() => {
    ...
    const loadImage = async () => {
      const url = await getIPFSUrl(nft.originalHash || nft.previewHash);
      setImageUrl(url);
      setLoadingImg(false);
    };
  ...
  return (
    ...
    >
      ...
      <View style={styles.nftInfo}>
        ...
        <Text style={[styles.nftDescription, { color: colors.textSecondary }]} numberOfLines={1}>
          Hash: {nft.originalHash}
        </Text>
        ...
        <View style={styles.nftDetails}>
          ...
          <TouchableOpacity
            ...
            onPress={() => navigation.navigate("Market", { nft })}
          >
...
// ---------- Main Screen ----------
const MyNFTScreen = () => {
  const { account, fetchMyNFTs, myNFTs, loading, error } = useContext(chatAppContext);
  const { colors } = useTheme();
  const navigation = useNavigation();
  ...
  if (loading && !refreshing) {
    return (
      <View style={[styles.loadingContainer, { backgroundColor: colors.background }]}>
        ...
        <Text style={[styles.loadingText, { color: colors.textSecondary }]}>Loading your NFTs...</Text>
      </View>
    );
  }
  ...
  if (error || localError) {
    return (
      <View style={[styles.errorContainer, { backgroundColor: colors.background }]}>
        ...
        <TouchableOpacity style={[styles.retryButton, { backgroundColor: colors.primary }]} onPress={fetchNFTs}>
  ...
  return (
    <Animated.View
      style={[
        styles.container,
        { opacity: fadeAnim, transform: [{ translateY }], backgroundColor: colors.background },
      ]}
    >
      ...
      {!formattedNFTs || formattedNFTs.length === 0 ? (
        <View style={styles.emptyContainer}>
          ...
          <TouchableOpacity style={[styles.createButton, { backgroundColor: colors.primary }]} onPress={() => navigation.navigate("CreateNFT")}>
...
const styles = StyleSheet.create({
  ...
  nftCard: { borderRadius: 16, overflow: "hidden", ...Platform.select({ ios: { shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 4 }, android: { elevation: 4 } }) },
  nftImage: { width: "100%", height: 200, resizeMode: "cover" },
  nftInfo: { padding: 16, gap: 8 },
  nftTitle: { fontSize: 18, fontWeight: "600" },
  nftDescription: { fontSize: 14, lineHeight: 20 },
  nftDetails: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginTop: 8 },
  priceContainer: { flexDirection: "row", alignItems: "center", gap: 4 },
  price: { fontSize: 16, fontWeight: "500" },
  sellButton: { flexDirection: "row", alignItems: "center", paddingVertical: 8, paddingHorizontal: 12, borderRadius: 20, gap: 4 },
  sellButtonText: { fontSize: 14, fontWeight: "500" },
  loadingContainer: { flex: 1, justifyContent: "center", alignItems: "center", gap: 16 },
  loadingText: { fontSize: 16 },
  errorContainer: { flex: 1, justifyContent: "center", alignItems: "center", gap: 16, padding: 16 },
  errorText: { fontSize: 16, textAlign: "center" },
  retryButton: { flexDirection: "row", alignItems: "center", paddingVertical: 12, paddingHorizontal: 24, borderRadius: 24, gap: 8 },
  retryButtonText: { color: "#fff", fontSize: 16, fontWeight: "600" },
  emptyContainer: { flex: 1, justifyContent: "center", alignItems: "center", gap: 16, padding: 16 },
